                                ...
                                return (
                                    <div className="order-item rounded-[4px] shadow-lg mb-12" key={`order-item-${index}`}>
                                                    ...
                                                    return (
                                                        <div className="order-product flex items-center justify-between pb-4 mb-4 border-b border-gray-300" key={`order-product-${index}-${product_index}`}>
                                                            <div className="order-product__info flex gap-x-4">
                                                                {/* <div className="w-24 h-24"><img src={product.product_image} alt="" /></div> */}
                                                                {(product.product_image !== "") ?
                                                                    <img src={`data:image/jpeg;base64,${product.product_image}`} alt='' className="w-24 h-24 cursor-pointer" />
                                                                    :
                                                                    <PiImageThin className="w-24 h-24 cursor-pointer text-gray-300" />
                                                                }
                                                                <div className="flex flex-col gap-y-1">
                                                                    <div className="font-medium">{product.product_name}</div>
                                                                    <div>x{product.quantity}</div>
                                                                    <div className="px-2 py-1 border border-green-500 text-green-500 w-fit">{product.quantity * product.price}</div>
                                                                </div>
                                                            </div>
                                        ...
                                        <div className="order-item__bottom bg-[#FFFEFA] px-3 py-2">
                                            <div className="my-6 text-end w-full">Thành tiền: <span className="text-xl text-[#FCB800] font-medium">{CurrencyFormat(item.totalPrice)}</span></div>
