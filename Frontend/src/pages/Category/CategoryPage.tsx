...
const CategoryPage = () => {
    ...
    const handlePageClick = (event) => {
        ...
        navigate({
            pathname: "/category",
            search: `?id=${categoryID}&page=${+event.selected + 1}&rating=${ratingSort}&minPrice=${minPrice}&maxPrice=${maxPrice}`,
    ...
    const fetchProductsByCategory = async (category_id: number, rating_sort: number, min_price: number, max_price: number) => {
        let response: IData = await getProductsByCategory(+category_id, +currentPage, rating_sort, min_price, max_price);
        if (response) {
    ...
    const handleRatingFilter = (rating_value: number) => {
        ...
        navigate({
            pathname: "/category",
            search: `?id=${categoryID}&page=1&rating=${rating_value}&minPrice=${minPrice}&maxPrice=${maxPrice}`,
    ...
    const handlePriceFilter = () => {
        ...
        if (maxPrice !== 0 && minPrice === 0) {
            navigate({
                pathname: "/category",
                search: `?id=${categoryID}&page=1&rating=${ratingSort}&maxPrice=${maxPrice}`,

            }, {
                replace: true
            });
            window.scrollTo({ top: 0, left: 0, behavior: 'smooth' });
            fetchProductsByCategory(categoryID, ratingSort, minPrice, maxPrice);
        } else if (maxPrice === 0 && minPrice !== 0) {
            navigate({
                pathname: "/category",
                search: `?id=${categoryID}&page=1&rating=${ratingSort}&minPrice=${minPrice}`,
            ...
            navigate({
                pathname: "/category",
                search: `?id=${categoryID}&page=1&rating=${ratingSort}&minPrice=${minPrice}&maxPrice=${maxPrice}`,
    ...
    const handleRemoveFilter = () => {
        navigate({
            pathname: "/category",
            search: `?id=${categoryID}&page=1&rating=0&minPrice=0&maxPrice=0`,
                            ...
                            <div className="main w-[80rem] mx-auto px-[30px] flex gap-x-3">
                                <div className="main__filter-sidebar w-60 pr-4 py-3 rounded-[4px] bg-[#F5F5F5] h-fit">
                                    ...
                                    <div className="section">
                                        ...
                                        <div className="my-2 w-full py-2 text-center border-2 rounded-[4px] border-[#FCB800] bg-white text-[#FCB800] hover:bg-[#FCB800] hover:text-white cursor-pointer" onClick={() => handlePriceFilter()}>Áp dụng</div>
                                    </div>
                                    <div className="section-breakline border-t border-gray-300 my-4"></div>
                                    <div className="section">
