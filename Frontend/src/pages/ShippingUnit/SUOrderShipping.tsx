...
import { confirmReceiveOrderSeller, getOrderStatus, handleShippingOrder } from "@/services/shippingUnitService";
...
const ORDER_STATUS = 4

const SUOrderShipping = () => {
    ...
    const handleOrderShipping = async (order_id: number) => {

        // order_status: 6 - Đang giao hàng

        let result = await handleShippingOrder(order_id);
        if (result) {
            if (result.EC === 0) {
    ...
    return (
        <>
            <div className="search-bar flex items-center justify-between mb-4">
                <div className="text-xl w-1/3">Giao hàng</div>
            ...
            <Modal show={showConfirmBox} setShow={setShowConfirmBox} size="delete-confirmation-box">
                <div className="delete-confirmation-box w-full h-full relative flex flex-col justify-between">
                    <div className="text-xl">Giao đơn hàng #{updateOrder}</div>
                    <div className="flex items-center justify-end gap-x-2 transition-all">
                        ...
                        <Button styles="rounded-[4px] px-8 py-2 bg-green-500 text-white hover:bg-green-600 cursor-pointer duration-200 rounded" OnClick={() => handleOrderShipping(updateOrder)}>Xác nhận</Button>
                    </div>
                </div>
            </Modal>
        </>
...
export default SUOrderShipping;