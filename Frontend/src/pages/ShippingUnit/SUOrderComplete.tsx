...
import { confirmReceiveOrderSeller, getOrderStatus, handleCompleteShippingOrder, handleShippingOrder } from "@/services/shippingUnitService";
...
const ORDER_STATUS = 6

const SUOrderComplete = () => {
    ...
    const handleCompleteOrderShipping = async (order_id: number) => {

        // order_status: 7 - Giao hàng thành công

        let result = await handleCompleteShippingOrder(order_id);
        if (result) {
            if (result.EC === 0) {
    ...
    return (
        <>
            <div className="search-bar flex items-center justify-between mb-4">
                <div className="text-xl w-1/3">Xác nhận giao hàng</div>
            ...
            <Modal show={showConfirmBox} setShow={setShowConfirmBox} size="delete-confirmation-box">
                <div className="delete-confirmation-box w-full h-full relative flex flex-col justify-between">
                    <div className="text-xl">Xác nhận đơn hàng <span className="text-red-500">#{updateOrder}</span> đã giao</div>
                    <div className="flex items-center justify-end gap-x-2 transition-all">
                        ...
                        <Button styles="rounded-[4px] px-8 py-2 bg-green-500 text-white hover:bg-green-600 cursor-pointer duration-200 rounded" OnClick={() => handleCompleteOrderShipping(updateOrder)}>Đã giao</Button>
                    </div>
                </div>
            </Modal>
        </>
...
export default SUOrderComplete;