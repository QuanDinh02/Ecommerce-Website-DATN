...
const getSubCategoryByCategoryWeb = async (req, res) => {
    try {
        let { id } = req.query
        let result = await subCategoryServices.getSubCategoryByCategoryWeb(+id);
        return res.status(200).json({
            EC: result.EC,
            DT: result.DT,
            EM: result.EM
...
module.exports = {
    getSubCategoryByCategory, getSubCategoryInfo, getRecommendSubCategory, getSubCategoryByCategoryWeb
}