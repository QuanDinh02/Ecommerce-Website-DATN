...
const deleteCartItemByCustomer = async (req, res) => {
    try {
        let { id } = req.params;
        let result = await cartItemServices.deleteCartItemByCustomer(id);
        return res.status(200).json({
            EC: result.EC,
            DT: result.DT,
            EM: result.EM
...
module.exports = {
    getQuickCartItems, addCartItem, updateCartItem, deleteCartItem, deleteCartItemByCustomer
}